...
{
    const
    { transferFrom, transferTo, amount, transferDate, note } = req.body;
    try
    {
        await transferMoney(transferFrom, transferTo, amount, note);
        res.redirect(`/account_dashboard?accountnum=${transferFrom}&message=Transfer completed successfully.`);
        
    }
    catch (error)
    {
         res.redirect(`/account_dashboard?accountnum=${transferFrom}&message=${encodeURIComponent(error.message)}`);
    }
});
...
{
  console.log("Server started successfully via localhost.");
});